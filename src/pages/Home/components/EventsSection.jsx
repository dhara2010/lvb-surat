          ...
          {!loading && !error && events.map((e, i) => (
            <SlideUp delay={i * 0.15} key={i} className="h-full">
              <Link to={`/events/${e.id || i}`} className="block h-full">
                <div className="bg-white/40 backdrop-blur-xl rounded-2xl overflow-hidden shadow-xl md:shadow-2xl border border-white/20 flex flex-col relative group h-full hover:border-[#0EA5A8]/50 hover:shadow-[0_12px_40px_rgba(14,165,168,0.15)] transition-all duration-300 cursor-pointer">
                  {/* Top Image Banner */}
                  ...
                  <div className="flex flex-col sm:flex-row p-5 md:p-6 relative flex-grow bg-white items-start sm:items-stretch">
                    ...
                    <div className="sm:ml-5 flex flex-col justify-center w-full">
                      <h3 className="font-extrabold text-base md:text-lg mb-2.5 leading-snug line-clamp-2 md:line-clamp-none group-hover:text-[#0EA5A8] transition-colors">{e.title}</h3>
                      <div className="flex flex-col gap-1.5 mt-auto">
                        <p className="text-[11px] md:text-xs font-semibold flex items-center gap-2">
                          <Calendar className="w-3.5 h-3.5 shrink-0 text-[#0EA5A8]" /> <span className="line-clamp-1 text-[#1F2937]">Every Wednesday</span>
                        </p>
                        <p className="text-[11px] md:text-xs font-semibold flex items-center gap-2">
                          <Target className="w-3.5 h-3.5 shrink-0 text-[#0EA5A8]" /> <span className="line-clamp-1 text-[#1F2937]">Surat, Gujarat</span>
                        </p>
                      </div>
                    </div>
                  </div>
