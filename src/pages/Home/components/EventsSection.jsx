    ...
    <div className="py-16 md:py-20 lg:py-24">
      <div className="container-xl px-4 mx-auto w-full max-w-[100vw] overflow-hidden">
        ...
        <div className="mt-8 md:mt-10 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 min-h-[300px] md:min-h-[400px]">
          {loading && <div className="col-span-full flex justify-center items-center text-muted text-sm md:text-base">Loading Events...</div>}
          {error && <div className="col-span-full flex justify-center items-center text-red-500 text-sm md:text-base">Failed to load events.</div>}
          {!loading && !error && events.length === 0 && <div className="col-span-full flex justify-center items-center text-muted text-sm md:text-base">No events available</div>}
          {!loading && !error && events.map((e, i) => (
            <ScrollReveal3D delay={i * 0.15} key={i} className="h-full">
              <TiltCard tiltMax={8} scaleMax={1.02} className="h-full">
                <Link to={`/events/${e.id || i}`} className="block h-full cursor-pointer">
                  <div className="bg-white rounded-2xl overflow-hidden shadow-[0_10px_30px_rgba(9,71,95,0.08)] border border-primary/5 flex flex-col relative group h-full hover:border-secondary/50 hover:shadow-[0_15px_40px_rgba(14,165,168,0.15)] transition-all duration-300">
                    {/* Top Image Banner */}
