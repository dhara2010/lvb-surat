          ...
          >
            <div className="flex justify-between items-start pt-1 pb-5 border-b border-border">
              <div className="flex flex-col gap-2">
                <h4 className="font-extrabold text-xl">{d.name}</h4>
                <div className="flex flex-wrap gap-2 text-xs font-bold tracking-wider">
                  <span className="bg-bg-alt border border-border px-3 py-1.5 rounded-lg">{d.email}</span>
                  <span className="bg-bg-alt border border-border px-3 py-1.5 rounded-lg">{d.phone}</span>
                </div>
              </div>
              <span className="text-[10px] uppercase tracking-widest  mt-1 font-bold shrink-0 text-right">
                {new Date(d.created_at).toLocaleDateString(undefined, {month:'short', day:'numeric', year:'numeric'})}
              </span>
            </div>
            <p className="text-sm leading-relaxed whitespace-pre-wrap px-1 font-medium">{d.message}</p>
          </motion.div>
        ))}
      </div>
      {data.length === 0 && <p className="p-10 text-center border border-dashed border-border rounded-2xl bg-white shadow-sm mt-4 font-semibold">Inbox is clean.</p>}
    </div>
  );
}
