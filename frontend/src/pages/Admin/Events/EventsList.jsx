      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-border pb-6">
        <div>
          <div className="text-xs font-bold uppercase tracking-wider  mb-2 flex items-center gap-2">
            <span>Admin Dashboard</span> <span>/</span> <span className="">Events Database</span>
          </div>
          <SectionHeader title="Events" desc="Manage robust event details and configurations." />
        </div>
        <button 
          onClick={() => navigate('/admin/events/add')}
          className="h-[44px] shrink-0 bg-primary  px-6 rounded-xl font-bold uppercase tracking-wider hover:bg-primary-light transition-all shadow-md flex items-center justify-center gap-2"
        >
      ...
      <PremiumTable 
        ...
        rows={data.map(d => (
          <tr key={d.id} className="border-b border-border hover:bg-bg-alt transition-colors">
            <td className="p-4 font-black text-lg  w-40">
            ...
            <td className="p-4 font-medium">
              ...
              <div className="text-xs bg-bg-alt inline-block px-2 py-0.5 mt-1 rounded border border-border">
