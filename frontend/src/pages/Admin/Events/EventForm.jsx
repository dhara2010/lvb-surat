  ...
  return (
    <div className="bg-white border border-border p-6 rounded-2xl shadow-sm">
      <form onSubmit={handleSubmit} className="flex flex-col gap-6">
        <h3 className="font-bold uppercase tracking-wider text-sm border-b border-border pb-2">Basic Info</h3>
        <div className="flex flex-col md:flex-row gap-4 items-end">
          ...
          <div className="flex flex-col gap-1.5 flex-1 w-full">
            ...
            <div className="flex items-center gap-2">
              <input type="text" value={form.image||''} onChange={e=>setForm({...form, image: e.target.value})} placeholder="/12-1.webp" className="flex-1 bg-bg-alt border border-border p-3 rounded-xl  outline-none focus:border-secondary focus:bg-white transition-all font-medium text-sm" />
              <label className="cursor-pointer bg-primary/10  hover:bg-primary/20 px-4 py-3 rounded-xl transition-all font-bold text-xs flex items-center justify-center whitespace-nowrap h-[46px]">
                Upload
        ...
        <div className="mt-2">
          <div className="flex items-center justify-between border-b border-border pb-2 mb-4">
            ...
            <button type="button" onClick={addSession} className="text-xs font-bold uppercase tracking-wider bg-bg-alt  hover:bg-slate-200 px-4 py-2 rounded-lg transition-colors flex items-center gap-1">+ Add Session</button>
          </div>
          <div className="flex flex-col gap-4">
            {form.sessions.map((s, i) => (
              <div key={i} className="bg-bg-alt p-5 rounded-xl border border-border relative shadow-sm">
                <button type="button" onClick={() => rmSession(i)} className="absolute top-4 right-4  hover:bg-red-50 px-3 py-1.5 rounded-lg text-xs font-bold tracking-wider transition-colors">X Remove</button>
                <div className="flex flex-wrap gap-4 mt-6">
                   <div className="flex flex-col gap-1.5 w-[140px]">
                      <label className="text-xs font-bold uppercase tracking-wider pl-1">Icon Type</label>
                      <select value={s.iconType} onChange={e => updateSession(i,'iconType', e.target.value)} className="w-full bg-white border border-border p-3 rounded-xl  outline-none focus:border-secondary font-medium text-sm">
        ...
        <div className="mt-2">
          <div className="flex items-center justify-between border-b border-border pb-2 mb-4">
            ...
            <button type="button" onClick={addTicket} className="text-xs font-bold uppercase tracking-wider bg-bg-alt  hover:bg-slate-200 px-4 py-2 rounded-lg transition-colors flex items-center gap-1">+ Add Ticket</button>
          </div>
          <div className="flex flex-col gap-4">
            {form.tickets.map((t, i) => (
              <div key={i} className="flex flex-col md:flex-row gap-4 items-start bg-bg-alt p-5 rounded-xl border border-border relative">
                ...
                <button type="button" onClick={() => rmTicket(i)} className="bg-red-50  px-4 py-3 md:mt-[22px] rounded-xl hover:bg-red-100 transition-all font-bold self-end md:self-auto">X</button>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex items-center gap-3 pt-6 border-t border-border mt-2">
          ...
          <button type="button" onClick={onCancel} className="h-[44px] px-6 rounded-xl font-bold uppercase tracking-wider bg-bg-alt  hover:bg-slate-200 transition-all">
            Cancel
          </button>
        </div>
      </form>
