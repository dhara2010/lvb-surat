...
export const SectionHeader = ({ title, desc }) => (
  <div className="mb-6">
    <h1 className="text-2xl md:text-3xl font-extrabold mb-1 text-heading">
      {title}
    </h1>
...
export const InputGroup = ({
  ...
  <div className={`flex flex-col gap-1.5 ${w}`}>
    <label className="text-xs font-bold uppercase tracking-wider pl-1 text-muted">
      {label}
    </label>
    <input
      required={req}
...
export const SubmitButton = ({ editing }) => (
  <button
    type="submit"
    className={`h-[44px] shrink-0 bg-cyan-600 hover:bg-cyan-505 text-white px-6 rounded-xl font-bold uppercase tracking-wider transition-all shadow-md flex items-center justify-center gap-2 ${editing ? "!bg-amber-600 hover:!bg-amber-500" : ""}`}
  >
    {editing ? (
...
export const EditBtn = ({ onClick }) => (
  <button
    type="button"
    onClick={onClick}
    className="text-cyan-400 hover:bg-cyan-500/10 p-2.5 rounded-lg transition-all mr-1"
    aria-label="Edit"
  >
    <Edit2 size={18} />
  </button>
...
export const DeleteBtn = ({ onClick }) => (
  <button
    type="button"
    onClick={onClick}
    className="text-rose-500 hover:bg-rose-500/10 p-2.5 rounded-lg transition-all"
    aria-label="Delete"
  >
    <Trash2 size={18} />
  </button>
...
export const PremiumTable = ({ headers, rows, emptyText }) => (
  <div className="bg-bg-alt border border-border rounded-2xl overflow-hidden shadow-sm">
        ...
        <thead className="bg-bg border-b border-border">
          <tr>
            {headers.map((h, i) => (
              <th
                key={i}
                className="p-4 text-xs font-bold uppercase tracking-widest text-muted"
              >
                {h}
              </th>
            ))}
    ...
    {rows.length === 0 && (
      <div className="p-10 text-center text-sm font-semibold text-muted">
        {emptyText}
      </div>
    )}
  </div>
...
export const resolveImageUrl = (url) => {
  ...
  */
  ...
  /*
    ...
    /uploads/gallery/xxx.webp

    These files live on backend.
  */
  ...
  /*
    ...
    Keep relative because these may be
    bundled/static frontend assets.

    NOTE:
    Old files must physically exist on
